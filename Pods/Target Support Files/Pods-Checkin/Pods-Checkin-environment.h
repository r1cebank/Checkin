
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// Canvas
#define COCOAPODS_POD_AVAILABLE_Canvas
#define COCOAPODS_VERSION_MAJOR_Canvas 0
#define COCOAPODS_VERSION_MINOR_Canvas 1
#define COCOAPODS_VERSION_PATCH_Canvas 2

// MKInputBoxView
#define COCOAPODS_POD_AVAILABLE_MKInputBoxView
#define COCOAPODS_VERSION_MAJOR_MKInputBoxView 0
#define COCOAPODS_VERSION_MINOR_MKInputBoxView 9
#define COCOAPODS_VERSION_PATCH_MKInputBoxView 3

// PXAlertView
#define COCOAPODS_POD_AVAILABLE_PXAlertView
#define COCOAPODS_VERSION_MAJOR_PXAlertView 0
#define COCOAPODS_VERSION_MINOR_PXAlertView 1
#define COCOAPODS_VERSION_PATCH_PXAlertView 1

// SwiftSpinner
#define COCOAPODS_POD_AVAILABLE_SwiftSpinner
#define COCOAPODS_VERSION_MAJOR_SwiftSpinner 0
#define COCOAPODS_VERSION_MINOR_SwiftSpinner 6
#define COCOAPODS_VERSION_PATCH_SwiftSpinner 7

// XCGLogger
#define COCOAPODS_POD_AVAILABLE_XCGLogger
#define COCOAPODS_VERSION_MAJOR_XCGLogger 2
#define COCOAPODS_VERSION_MINOR_XCGLogger 0
#define COCOAPODS_VERSION_PATCH_XCGLogger 0

